  ...
  for (lg in c(FALSE, TRUE)) {
    for (lt in c(TRUE, FALSE)) {
      for (i in seq_len(3L)) {
        ...
        if (lg) pt <- log(pt)
        q_trunc <- qtrunc(pt, fam, rate, lower.tail = lt, log.p = lg, a = a, b = b)
        q_stats <- qexp(pt, rate, lower.tail = lt, log.p = lg)
        expect_length(q_trunc, i)
        for (ii in seq_along(pt)) {
          # Working back to p from q
          ptr <- ptrunc(q_trunc[ii], fam, rate, lower.tail = lt, log.p = lg, a = a, b = b)
          expect_equal(pt[ii], ptr)
        }
      }
    }
  ...
  for (lg in c(FALSE, TRUE)) {
    for (lt in c(TRUE, FALSE)) {
      for (i in seq_len(3L)) {
        ...
        b <- max(ab)
        if (lg) pt <- log(pt)
        q_trunc <- qtrunc(pt, fam, shp, rte, lower.tail = lt, log.p = lg, a = a, b = b)
        q_stats <- qgamma(pt, shp, rte, lower.tail = lt, log.p = lg)
        expect_length(q_trunc, i)
        for (ii in seq_along(pt)) {
          # Working back to p from q
          ptr <- ptrunc(q_trunc[ii], fam, shp, rte, lower.tail = lt, log.p = lg, a = a, b = b)
          expect_equal(pt[ii], ptr)
        }
      }
    }
  ...
  for (lg in c(FALSE, TRUE)) {
    for (lt in c(TRUE, FALSE)) {
      for (i in seq_len(3L)) {
        ...
        b <- qtrunc(sqrt(max(pt)), fam, sz, pb, lower.tail = TRUE, log.p = FALSE)
        if (lg) pt <- log(pt)
        q_trunc_pb <- qtrunc(
