...
validateDomain.beta <- function(n, shape1, shape2, ...) {
	if (shape1 < 0) stop("Invalid parameter domain. shapes1 must be non-negative.")
	if (shape2 < 0) stop("Invalid parameter domain. shapes2 must be non-negative.")
}
...
validateDomain.gamma <- function(n, shape, rate, ...) {
	...
	# TODO: add validation for scale (and don't check the one that isn't used)
}
...
validateDomain.invgauss <- function(n, m, s, ...) {
	if (m <= 0) stop("Invalid parameter domain. m must be > 0.")
	if (s <= 0) stop("Invalid parameter domain. s must be > 0.")
}

validateDomain.lognormal <- function(n, meanlog, sdlog, ...) {
	if (is.complex(meanlog)) stop("Invalid parameter domain. meanlog must be real.")
	if (sdlog <= 0) stop("Invalid parameter domain. rate must be > 0.")
}
...
validateDomain.nbinom <- function(n, size, prob, mu, ...) {
	...
	if (is.complex(mu)) stop("Invalid parameter domain. mean must be real.")
}

validateDomain.normal <- function(n, mean, sd, ...) {
	if (is.complex(mean)) stop("Invalid parameter domain. mean must be real.")
	if (sd <= 0) stop("Invalid parameter domain. sd must be > 0.")
}
